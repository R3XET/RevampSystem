...
public class UnBlacklistCommand extends BaseCommand {
    ...
    @Command(name = "unblacklist", inGameOnly = false, permission = "punishments.command.unblacklist", aliases = {"unbl", "unblplayer", "unblacklistplayer"}) @SuppressWarnings("deprecation")
    public void onCommand(CommandArgs command) {
        ...
        Tasks.runAsync(plugin, () -> {
            if (args.length < 2) {
                sender.sendMessage(PunishmentsLanguage.UNBLACKLIST_USAGE.toString());
                return;
            }
