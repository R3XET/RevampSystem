...
import eu.revamp.system.menus.rank.RankImportMenu;
import eu.revamp.system.menus.rank.RankListMenu;
...
public class RankCommand extends BaseCommand {
    ...
    @Command(name = "rank", permission = "revampsystem.command.rank", aliases = {"ranks"}, inGameOnly = false)
    public void onCommand(CommandArgs command) {
        Tasks.runAsync(plugin, () -> {
            ...
            if (args.length == 1) {
                if (args[0].equalsIgnoreCase("list")) {
                    if (player instanceof Player) {
                        if (plugin.getCoreConfig().getBoolean("RANK_LIST_USE_GUI")) {
                    ...
                    plugin.getRankManagement().getRanks().stream().sorted(Comparator.comparingInt(RankData::getWeight).reversed()).forEach(rankData -> {
                        plugin.getCoreConfig().getStringList("RANK_LIST_FORMAT").forEach(message -> {
                            ...
                            replacement.add("%permissions%", rankData.getPermissions().size() == 0 ? "None" :
                                    eu.revamp.spigot.utils.string.StringUtils.getStringFromList(rankData.getPermissions()));
            ...
            if (args.length == 2) {
                ...
                if (args[0].equalsIgnoreCase("listpermissions") || args[0].equalsIgnoreCase("listperm")) {
                    ...
                    player.sendMessage(Language.RANK_PERMISSIONS.toString()
                            .replace("%rank%", rankData.getName())
                            .replace("%permissions%", eu.revamp.spigot.utils.string.StringUtils.getStringFromList(rankData.getPermissions()))
                ...
                if (args[0].equalsIgnoreCase("listbpermissions") || args[0].equalsIgnoreCase("listbperm")) {
                    ...
                    player.sendMessage(Language.RANK_PERMISSIONS.toString()
                            .replace("%rank%", rankData.getName())
                            .replace("%permissions%", eu.revamp.spigot.utils.string.StringUtils.getStringFromList(rankData.getBungeePermissions()))
            ...
            if (args[0].equalsIgnoreCase("setprefix")) {
                ...
                String prefix = Color.translate(CCUtils.buildMessage(args, 2));
            ...
            if (args[0].equalsIgnoreCase("setsuffix")) {
                ...
                String suffix = Color.translate(CCUtils.buildMessage(args, 2));
            ...
            if (args.length == 3) {
                ...
                if (args[0].equalsIgnoreCase("settype")) {
                    ...
                    player.sendMessage(Language.RANK_TYPE_SET.toString()
                            .replace("%rank%", args[1])
                            .replace("%type%", eu.revamp.spigot.utils.string.StringUtils.convertFirstUpperCase(args[2])));
                ...
                if (args[0].equalsIgnoreCase("setcoinscost")) {
                    ...
                    player.sendMessage(Language.RANK_COINS_COST_SET.toString()
                            .replace("%rank%", args[1])
                            .replace("%tamountype%", String.valueOf(rankData.getCoinsCost())));
    ...
    private void sendFirstPage(CommandSender player) {
        ...
        player.sendMessage(Color.translate("&e/rank list &8- &7see available ranks " + (plugin.getCoreConfig().getBoolean("RANK_LIST_USE_GUI") ? "&b(RevampMenu)" : "") + "&7."));
        player.sendMessage(Color.translate("&7&m---------------------------------------------"));
    }
    ...
    private void sendThirdPage(CommandSender player) {
        ...
        player.sendMessage(Color.translate("&e/rank import &8- &7import ranks &b(RevampMenu)&7."));
