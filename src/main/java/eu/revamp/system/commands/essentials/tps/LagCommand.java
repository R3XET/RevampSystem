package eu.revamp.system.commands.essentials.tps;
/*
...
public class LagCommand extends BaseCommand {
    ...
    @Command(name = "lag", permission = "revampsystem.command.lag", aliases = {"lagg"}, inGameOnly = false)
    public void onCommand(CommandArgs command) {
        Tasks.runAsync(plugin, () -> {
            CommandSender player = command.getSender();

            plugin.getCoreConfig().getStringList("TPS_COMMAND_FORMAT").forEach(message -> {
                if (!message.contains("%worlds%")) {
                    ...
                    Bukkit.getWorlds().forEach(world -> {
                        plugin.getCoreConfig().getStringList("TPS_COMMAND_WORLD_FORMAT").forEach(msg -> {
                            ...
                            replacement.add("%livingEntitiesdChunks%", world.getLivingEntities().size());
