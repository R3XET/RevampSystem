...
public class TagManagement extends Manager {
    ...
    public TagManagement(RevampSystem plugin) {
        super(plugin);
    ...
    public void saveTags() {
        tags.forEach(tag -> {
            ...
            document.put("color", CCUtils.convertChatColor(tag.getColor()));
    ...
    public void saveTagsToConfig() {
        ...
        this.tags.stream().sorted(Comparator.comparingInt(Tag::getWeight).reversed()).forEach(tag -> {
            ...
            file.set(tag.getName() + ".color", CCUtils.convertChatColor(tag.getColor()));
