...
public class FilterManagement extends Manager {
    ...
    public FilterManagement(RevampSystem plugin) {
        super(plugin);
    ...
    public boolean checkFilter(Player player, String message, boolean sendMessage) {
        if (player.hasPermission("revampsystem.filter.bypass")) return false;

        if (this.isFiltered(message)) {
            if (sendMessage) {
                player.sendMessage(Language.FILTER_CANT_SEND.toString());
            }
            plugin.getServerManagement().getGlobalPlayers().stream().filter(globalPlayer -> globalPlayer.hasPermission("revampsystem.filter.alerts")).forEach(globalPlayer -> {
