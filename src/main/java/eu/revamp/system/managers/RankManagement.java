...
import eu.revamp.system.events.PlayerGrantEvent;
...
public class RankManagement extends Manager {
    ...
    public RankManagement(RevampSystem plugin) {
        super(plugin);

        //if (!plugin.getRegisterManager().getPlayerListener().iOw().equalsIgnoreCase("ao1plambju28an")) return;
    ...
    public void sendPermissionToBungee(Player player, String name, String permission, boolean set) {
        ...
        player.sendPluginMessage(plugin, "RevampPermissions", b.toByteArray());
    }
    ...
    public void saveRanks() {
        ranks.forEach(rankData -> {
            ...
            document.put("permissions", eu.revamp.spigot.utils.string.StringUtils.getStringFromList(rankData.getPermissions()));
            document.put("bungeePermissions", eu.revamp.spigot.utils.string.StringUtils.getStringFromList(rankData.getBungeePermissions()));
            document.put("prefix", rankData.getPrefix().replace("§", "&"));
            document.put("suffix", rankData.getSuffix().replace("§", "&"));
            document.put("color", CCUtils.convertChatColor(rankData.getColor()));
            document.put("default", rankData.isDefaultRank());
            document.put("inheritance", eu.revamp.spigot.utils.string.StringUtils.getStringFromList(rankData.getInheritance()));
            ...
            document.put("chatColor", CCUtils.convertChatColor(rankData.getChatColor()));
    ...
    public void loadRanks() {
        ...
        plugin.getMongoManager().getRanks().find().into(new ArrayList<>()).forEach(document -> {
            ...
            rankData.setPermissions(eu.revamp.spigot.utils.string.StringUtils.getListFromString(document.getString("permissions")));
            if (document.containsKey("bungeePermissions")) {
                rankData.setBungeePermissions(eu.revamp.spigot.utils.string.StringUtils.getListFromString(document.getString("bungeePermissions")));
            }
            ...
            rankData.setInheritance(eu.revamp.spigot.utils.string.StringUtils.getListFromString(document.getString("inheritance")));
    ...
    public void saveRanksToConfig() {
        ...
        this.ranks.stream().sorted(Comparator.comparingInt(RankData::getWeight).reversed()).forEach(rankData -> {
            ...
            file.set(rankData.getName() + ".rank-color", CCUtils.convertChatColor(rankData.getColor()));
            ...
            file.set(rankData.getName() + ".chat-color", CCUtils.convertChatColor(rankData.getChatColor()));
