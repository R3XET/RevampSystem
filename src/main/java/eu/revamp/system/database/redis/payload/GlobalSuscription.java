package eu.revamp.system.database.redis.payload;
...
import eu.revamp.system.database.redis.other.suscriber.handle.JedisHandle;
...
import eu.revamp.system.events.GlobalPlayerCreateEvent;
import eu.revamp.system.events.GlobalPlayerDestroyEvent;
...
public class GlobalSuscription implements JedisHandle {
    ...
    public void handleMessage(JsonObject object) {
        ...
        if (payload == JedisAction.SERVER_DATA) {
            ...
            serverData.setNames(eu.revamp.spigot.utils.string.StringUtils.getListFromString(data.get("players").getAsString()));
        ...
        if (payload == JedisAction.PLAYER_DATA) {
            ...
            globalPlayer.setPermissions(eu.revamp.spigot.utils.string.StringUtils.getListFromString(data.get("permissions").getAsString()));
        ...
        if (payload == JedisAction.SERVER_ONLINE) {
            ...
            PlayerUtils.getOnlinePlayers().stream().filter(player -> player.hasPermission("revampsystem.server.status.messages")).forEach(player -> {
        ...
        if (payload == JedisAction.PLAYER_PERMISSIONS_UPDATE) {
            ...
            if (player != null) {
                PlayerData playerData = plugin.getPlayerManagement().getPlayerData(player.getUniqueId());
                playerData.setPermissions(eu.revamp.spigot.utils.string.StringUtils.getListFromString(permissions));
        ...
        if (payload == JedisAction.PERMISSIONS_UPDATE) {
            Tasks.runAsync(plugin, () -> PlayerUtils.getOnlinePlayers().forEach(player -> {
                PlayerData playerData = plugin.getPlayerManagement().getPlayerData(player.getUniqueId());
        ...
        if (payload == JedisAction.STAFF_CONNECT) {
            ...
            PlayerUtils.getOnlinePlayers().stream().filter(player -> player.hasPermission("revampsystem.staff.join.messages")).forEach(player -> {
        ...
        if (payload == JedisAction.LEFT_FROZEN) {
            ...
            PlayerUtils.getOnlinePlayers().stream().filter(player -> player.hasPermission("revampsystem.command.freeze")).forEach(player -> {
        ...
        switch (payload) {
            case EXECUTE_UNBAN: {
                ...
                if (silent) {
                    for (Player player : PlayerUtils.getOnlinePlayers().stream().filter(player -> player.hasPermission("punishments.see.silent")).collect(Collectors.toList())) {
            ...
            case EXECUTE_PUNISHMENT: {
                ...
                if (type == PunishmentType.BAN) {
                    ...
                    if (IPRelative) {
                        eu.revamp.spigot.utils.string.StringUtils.getListFromString(alts).forEach(alt -> plugin.getRedisData().write(JedisAction.EXECUTE_ALT_KICK,
                                new JsonChain()
