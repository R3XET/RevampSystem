package eu.revamp.system.punishments.managers;
...
public class MessagesManager extends Manager {
    ...
    public MessagesManager(RevampSystem plugin) {
        super(plugin);
    }
    ...
    public void setup() {
        ...
        plugin.getPunishmentPlugin().getConfigFile().getConfigurationSection("BROADCASTS").getKeys(false).forEach(key -> {
            ...
            String hoverMessage = eu.revamp.spigot.utils.string.StringUtils.getStringFromList(plugin.getPunishmentPlugin().getConfigFile().getStringList("BROADCASTS." + key + ".HOVER.MESSAGE")).replace(", ", "\n");
        ...
        plugin.getPunishmentPlugin().getConfigFile().getConfigurationSection("KICK-MESSAGES").getKeys(false).forEach(key -> {
            this.kickMessages.put(key, eu.revamp.spigot.utils.string.StringUtils.getStringFromList(plugin.getPunishmentPlugin().getConfigFile().getStringList("KICK-MESSAGES." + key)));
        });
    }
